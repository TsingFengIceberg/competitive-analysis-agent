...
| 维度 | 要求 |
|------|------|
| Agent 数量 | 3-4 个（非固定，可有其他设计） |
| 编排框架 | LangGraph / CrewAI 推荐，**可手搓，不限制** |
| Agent 通信 | **结构化消息传递**（function calling / Schema），非纯自然语言 |
| 知识结构化 | 定义竞品知识 Schema（功能树、定价模型、用户画像），输出必须符合 |
| 反馈闭环 | 质检 Agent 可将不足打回采集 Agent，形成 DAG 式任务流转迭代闭环 |
| 信息溯源 | 每条分析结论标注数据来源，支持 traceability |
| 可观测 | 每个 Agent 决策过程与中间产物可追溯 |
| 问卷/访谈 | 采集 Agent 包含问卷设计、问卷调研、用户访谈（都行，能完全自动化更好） |
| 开源合规 | 只能使用 MIT / Apache 2.0 / BSD 协议的开源库 |
| 前端 | 推荐做前端展示页面，可现场演示 |
...
| 维度 | 权重 | 关键得分点 |
|------|------|-----------|
| 多 Agent 协作与输出可信度 | **35%** | 角色划分清晰、编排合理、结构化通信、真实反馈闭环、Schema 强制、溯源完整 |
| 技术深度与工程完整度 | **25%** | 端到端可演示、可观测、上下文管理/错误恢复/幻觉抑制、前瞻性思考 |
| 业务价值与产品体验 | **20%** | 效率/覆盖度/一致性可量化提升、贴合真实工作流、可扩展、交互流畅 |
| 代码质量与文档 | **10%** | 规范、模块化、注释、文档齐全、Git 规范、AI 编程工具使用痕迹 |
| 合规与答辩 | **10%** | 采集合规、数据脱敏、工具使用合规、答辩清晰 |
...
class CompetitionState(AgentState):
    ...
    # ── Writer 输出（普通模式）──
    final_report: str                   # Markdown (普通模式最终报告)
    traceability_map: dict              # claim_id → source_url
...
### 4.4 用户声音聚合器 (Voice of Customer Aggregator)

> **替代传统的"问卷分发"** — 更自动化、更客观、技术含量更高。
...
### 5.2 飞书审批 HITL Gate（P1 — 新增，比赛亮点）

当 Reviewer 发现需要人工判断的数据冲突时，不走传统的前端 HITL 页面，而是**复用飞书审批流**：
...
### 5.7 人对报告的细粒度交互式编辑（P0 — 答辩核心交互）

> **核心理念**：竞品分析不是"AI 生成 → 人看一眼"就完了。人要能像 Claude Code 里选中代码段一样，选中报告中的任意段落，然后说"这段深度调研一下"或"重写这部分"——甚至可以手动编辑。
...
> 评分标准 35%（多Agent协作与输出可信度）明确要求"DAG任务流转可视化、可追溯"。
