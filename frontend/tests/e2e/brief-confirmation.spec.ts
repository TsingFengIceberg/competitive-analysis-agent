...
const draftBrief = {
  ...
  time_range: { mode: "last_12_months", label: "最近12个月", start: null, end: null },
  dimensions: [{ id: "features", label: "功能与体验", weight: 0.5 }, { id: "pricing", label: "定价与商业模式", weight: 0.5 }],
  complexity: "standard",
  ...
  ambiguities: [{ field: "target_products", question: "请确认具体竞品名单。", required: true }],
  confidence: 0.5,
...
test.describe("Analysis Brief confirmation", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route("**/api/competition/comp-e2e/confirm", async (route) => {
      ...
      await route.fulfill({ json: { thread_id: "comp-e2e", status: "running", analysis_brief: { ...draftBrief, revision: 2, target_products: ["Cursor", "Copilot"], readiness: "ready", confirmation_source: "user" } } });
    });
    await page.route("**/api/competition/stream/comp-e2e", (route) => route.fulfill({ status: 200, contentType: "text/event-stream", body: "event: values\ndata: {\"status\":\"running\"}\n\n" }));
  });
