...
const report = {
  ...
  quality_gate: {
    ...
    dimensions: [
      {
        dimension_id: "features",
        label: "功能与体验",
        selected: true,
        products_total: 2,
        products_covered: ["Cursor"],
        missing_products: ["Copilot"],
        data_point_count: 1,
        source_domain_count: 1,
        coverage_ratio: 0.5,
        status: "blocked",
        issue_ids: ["coverage-features-1"],
      },
    ],
    sources: {
      total: 1,
    ...
    claims: { total: 1, multi_source: 0, single_source: 1, unsupported: 0 },
    issues: [
      {
        id: "warning-1",
        level: "warning",
        severity: "minor",
        type: "single_source",
        check_method: "comparison_claim",
        description: "声明只有单一来源",
        remediation: "补充独立来源",
        dimension_ids: [],
        product_names: [],
        data_point_ids: [],
        citation_ids: ["1"],
        section_ids: ["sec-comparison-matrix"],
      },
    ],
    rework: {
      review_round: 1,
      ...
      repair_delta: null,
      current_round_metrics: null,
      previous_round_metrics: null,
    },
  },
};
...
test.describe("Research workbench", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route(
      ...
      (route) => route.fulfill({ json: { history, count: 1 } }),
    );
    await page.route(
      "**/api/competition/report/workbench-thread/trace",
      (route) =>
        route.fulfill({
          json: {
            thread_id: "workbench-thread",
                ...
                phases: [
                  {
                    ...
                    content: {},
                    details: [],
                    json_output: { title: report.title },
                  },
                ],
              },
            ],
  ...
  test("opens the three-pane desktop workbench and source inspector", async ({
    page,
  }) => {
