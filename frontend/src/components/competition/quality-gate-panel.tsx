          ...
          {qualityGate.issues.length === 0 ? (
            ...
            qualityGate.issues.map((issue) => (
              <button
                ...
                className={`w-full rounded-lg border px-2.5 py-2 text-left transition-colors ${selectedIssueId === issue.id ? "border-primary bg-primary/5" : "bg-card hover:bg-muted/40"}`}
              >
