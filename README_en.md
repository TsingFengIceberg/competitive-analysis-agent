...
The default strict set, `evals/rag/real-v1.json`, contains human-curated snapshots of public first-party competitor documentation with source URLs, capture times, and expected labels. `basic-v1.json` remains an explicitly synthetic unit-regression set. Neither enters the business knowledge base. The command below runs ingestion, cost-routed query planning, retrieval, and claim verification against temporary SQLite and in-memory Qdrant stores. It reports Recall@5, MRR, NDCG@5, no-answer abstention accuracy, traceability completeness, P50/P95 latency, direct/multi-hop route accuracy, decomposition coverage, claim-status accuracy, contradiction recall, citation precision, numeric-consistency accuracy, groundedness, governance admission accuracy, quarantine recall, historical-memory recall, memory-to-evidence isolation, and current-thread exclusion, then writes the detailed report under the ignored `.ci-agent/evaluations/` directory:
...
- Extend the current entity events, cross-source clustering, three-layer insights, and document-review feedback with editable relationships, hypothesis approval/rejection, and historical accuracy tracking.
